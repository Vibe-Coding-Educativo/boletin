...
// abrir el enlace directo en su año y no en el seleccionado por defecto.
...
async function init() {
    try {
        ...
        if (selectedYear) {
            ...
            const yearToLoad = yearFromCard || selectedYear;

            if (yearToLoad !== selectedYear && yearSelector) {
                yearSelector.value = yearToLoad;
            }
...
function generateMediaEmbed(link, fullSize = false) {
    ...
    if (youtubeId) {
        if (fullSize) {
            ...
            // Vista previa con iframe (sin miniaturas estáticas)
            return `<div class=\"relative w-full\"><div class=\"relative pb-[56.25%] h-0\"><iframe class=\"absolute top-0 left-0 w-full h-full rounded-md shadow-md\" src=\"https://www.youtube-nocookie.com/embed/${youtubeId}\" title=\"YouTube video player\" frameborder=\"0\" loading=\"lazy\" referrerpolicy=\"strict-origin-when-cross-origin\" allowfullscreen sandbox=\"allow-same-origin allow-scripts allow-popups allow-popups-to-escape-sandbox allow-forms\"></iframe></div></div>`;
        }
    }
